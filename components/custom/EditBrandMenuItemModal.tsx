...
import { BrandMenuItem } from '@/types'
import { useUpdateBrandMenuItem } from '@/lib/supabase/client-cache'

interface EditBrandMenuItemModalProps {
  isOpen: boolean
  onClose: () => void
  item: BrandMenuItem | null
}

export function EditBrandMenuItemModal({ 
  isOpen, 
  onClose, 
  item
}: EditBrandMenuItemModalProps) {
  ...
  const updateBrandMenuItem = useUpdateBrandMenuItem()
  ...
  useEffect(() => {
    if (item && isOpen) {
      setFormData({
        ...
        price_cents: item.price_cents ? item.price_cents.toString() : '',
        serving_size: item.serving_size || '',
        kcal_per_serving: item.kcal_per_serving ? item.kcal_per_serving.toString() : '',
        g_protein_per_serving: item.g_protein_per_serving ? item.g_protein_per_serving.toString() : '',
        g_carb_per_serving: item.g_carb_per_serving ? item.g_carb_per_serving.toString() : '',
        g_fat_per_serving: item.g_fat_per_serving ? item.g_fat_per_serving.toString() : '',
        g_fiber_per_serving: item.g_fiber_per_serving ? item.g_fiber_per_serving.toString() : '',
        g_sugar_per_serving: item.g_sugar_per_serving ? item.g_sugar_per_serving.toString() : '',
        mg_sodium_per_serving: item.mg_sodium_per_serving ? item.mg_sodium_per_serving.toString() : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update the menu item using the mutation
      await updateBrandMenuItem.mutateAsync({
        itemId: item.id,
          ...
          <div className="p-6">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">Update {item.name}</p>
              </div>
              <Button
                type="button"
                variant="ghost"
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g., 1 item, 100g, 8 oz"
                    disabled={isSubmitting}
                    className="mt-1"
                  />
                </div>
              ...
              {/* Nutrition Info */}
              <div>
                <h4 className="text-sm font-medium text-gray-900 mb-3">Nutrition Information (per serving)</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    <Label htmlFor="calories" className="text-xs font-medium text-gray-700">
                      Calories
                    </Label>
                    <Input
                      id="calories"
                  ...
                  <div>
                    <Label htmlFor="protein" className="text-xs font-medium text-gray-700">
                      Protein (g)
                    </Label>
                    <Input
                      id="protein"
                  ...
                  <div>
                    <Label htmlFor="carbs" className="text-xs font-medium text-gray-700">
                      Carbs (g)
                    </Label>
                    <Input
                      id="carbs"
                  ...
                  <div>
                    <Label htmlFor="fat" className="text-xs font-medium text-gray-700">
                      Fat (g)
                    </Label>
                    <Input
                      id="fat"
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div>
                  <Label htmlFor="fiber" className="text-xs font-medium text-gray-700">
                    Fiber (g)
                  </Label>
                  <Input
                    id="fiber"
                ...
                <div>
                  <Label htmlFor="sugar" className="text-xs font-medium text-gray-700">
                    Sugar (g)
                  </Label>
                  <Input
                    id="sugar"
                ...
                <div>
                  <Label htmlFor="sodium" className="text-xs font-medium text-gray-700">
                    Sodium (mg)
                  </Label>
                  <Input
                    id="sodium"
